...
Podstawa: [scenariusz 0.3](SYNTHETIC_DATA_SCENARIO.md), [kontrakt 0.2](ANALYTICS_CONTRACT.md), [projekt BigQuery](BIGQUERY_DESIGN.md) oraz zaakceptowane [DDL GA4](../bigquery/ddl/001_create_ga4_events_demo.sql), [Meta](../bigquery/ddl/002_create_meta_daily_campaign_stats_demo.sql), [Google Campaign](../bigquery/ddl/003a_create_google_campaign_demo.sql), [BasicStats](../bigquery/ddl/003b_create_google_campaign_basic_stats_demo.sql), [ConversionStats](../bigquery/ddl/003c_create_google_campaign_conversion_stats_demo.sql), [Customer](../bigquery/ddl/003d_create_google_customer_demo.sql), [Profitroom](../bigquery/ddl/004_create_profitroom_reservations_demo.sql).
...
**Aktualizacja source:** wyjście przyszłego generatora to Campaign_demo (27 pól), CampaignBasicStats_demo (17), CampaignConversionStats_demo (20) i Customer_demo (9), zgodne 1:1 z query schema widoków ads_*_6604551350 w my-story-sopot.my_story_sopot_dataset. Wszystkie pola nullable, ID campaign_id/customer_id mają INT64. Metadane generatora pozostają w konfiguracji i manifeście, poza rekordami source. _LATEST_DATE i _DATA_DATE są źródłowymi polami DATE. DDL nie dodaje partycjonowania ani clusteringu. Implementacja generatora i reguły emisji snapshotów pozostają kolejnym etapem.

**G06 — ZAAKCEPTOWANE:** trzy syntetyczne kampanie — po jednej Search Generic, Brand i GHA, jeden syntetyczny customer_id. Nowe nazwy opisują ofertę fikcyjnego hotelu. Katalog przyszłego generatora dostarczy syntetyczne, deterministyczne ID INT64 zgodne ze źródłem; role Brand, Generic i GHA pozostają konfiguracją, a nie kolumną campaign_group. Nazwy i identyfikatory rzeczywistych kampanii pozostają poza generowanymi danymi. Alternatywne kampanie historycznej referencji nie stają się dwiema równoległymi kopiami kosztu.

Plan aktywności jest oddzielny od campaign_status na as_of_at. Propozycja bazowa: kampanie dostępne w całym oknie, z konfigurowalnymi przerwami emisji jako parametrem implementacji. Liczba rekordów zależy od źródłowych segmentów oraz dat snapshotów; wcześniejsze 270 wierszy nie jest docelową liczebnością source. Brak emisji oznacza znany zerowy koszt i delivery. Działania atrybucyjne w takim dniu zależą od polityki dat, a nie automatycznie od zera kosztu.
...
Koszt rozdzielany jest na role 60/35/5, następnie na dni. Wyświetlenia i kliknięcia korzystają z osobnych profili grup, inspirowanych proporcjami referencji. Konwersje każdej akcji otrzymują własne wagi i opóźnienia raportowania. Kredyty konwersji są eksportowane jako FLOAT64 zgodnie ze źródłem. Koszt pozostaje metrics_cost_micros INT64; cele PLN są punktami kalibracyjnymi, nie kolumną kosztu source. Dzienne nierówności step1 >= step2 >= step3 >= purchases nie są walidacją.

Telefon i Calls from ads mają oddzielne pule działań w CampaignConversionStats, rozpoznawane przez segments_conversion_action / category / name, bez własnych kolumn phone_contacts i calls_from_ads. Referencja Brand 8 i 2 nie jest pełnym wynikiem wszystkich grup; brak podstawy do generowania danego działania oznacza NULL zgodnie z G06. Brakujące dane w innych rolach pozostają NULL, zamiast zer lub samodzielnie ekstrapolowanych celów.

**G06 — ZAAKCEPTOWANE:** prosta, jawna i deterministyczna polityka raportowania, bez symulatora atrybucji lub aukcji. Model i okno pozostają lokalną konfiguracją, o ile nie mają odpowiedników źródłowych; nie dodajemy attribution_policy_id ani innych pól do source. Etapy i zakupy pozostają w wierszach CampaignConversionStats; metrics_conversions i metrics_conversions_value dotyczą wskazanej akcji. Nazwy purchases/purchase_value używane w zasadach wspólnego przydziału oznaczają pojęcia obliczeń wewnętrznych, nie nowe kolumny Google Ads. Zachowujemy wspólny przydział kredytu i wartości tej samej akcji oraz odrębność od Profitroom. Metryk BasicStats i ConversionStats nie sumuje się ze sobą jako niezależnych zakupów.
...
- Google Ads: customer_id i campaign_id mają syntetyczne wartości INT64; wcześniejsze tekstowe etykiety katalogu są aliasami konfiguracji. Eksport kosztu zachowuje jednostkę micros INT64, eksport kredytów i wartości konwersji FLOAT64. Metadane scenario/hotel/wersje pozostają w manifeście.
...
- Kwoty reklam i Profitroom są nieujemne, obliczane w groszach i zaokrąglane deterministycznie; źródłowy eksport Profitroom ma FLOAT64, Meta zachowuje NUMERIC zgodnie ze swoim DDL; Google Ads przechowuje koszt w metrics_cost_micros INT64, a wartości i kredyty konwersji w FLOAT64. GA4 zachowuje istniejący typ FLOAT64 dla opcjonalnej wartości, zgodnie z DDL i G04. Wynik w tolerancji nie wymaga trafienia w cel co do grosza.
...
| Google | Wszystkie cele bazowe i kredyty osobno, koszt z CampaignBasicStats z uwzględnieniem wszystkich segmentów źródłowych i dat snapshotów, bez mnożenia przez akcje CampaignConversionStats, sumy grup i mix 60/35/5, zgodność polityki i brak arbitralnych zer telefonów |
...
| Syntetyczność | is_synthetic=true tam, gdzie kolumna istnieje; Google Ads potwierdza pochodzenie w manifeście, bez dodawania kolumn; ID wyłącznie z generatora/katalogu; URL tylko .example; brak wejścia z rekordami klientów; kontrola pochodzenia ważniejsza niż sam prefiks ID |
